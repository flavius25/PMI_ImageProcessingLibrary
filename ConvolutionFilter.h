...
/* This header file contains ConvolutionFilter implementation
...
namespace fgm_library
{
    ...
    public:
        /* Constructor */
        ConvolutionFilter() : _r(0) {};                     //set radius to initial value 0  
    ...
    protected:
        ...
        virtual void execute(const Image& i) override {
            double sum = 0;                                         //for counting elements in the window
                                        ...
                                        for (int var_z = z - z_r; var_z < z + z_r +1; var_z++) {
                                            for (int var_y = y - y_r; var_y < y + y_r +1; var_y++) {
                                                for (int var_x = x - x_r; var_x < x + x_r +1; var_x++) {
                                                    ...
                                                    sum += i(var_x, var_y, var_z, var_c, var_t);        //calculate the sum of all the pixels in the window
                                                }
                                            }
                                        }
                                    }
                                }
                                _output(x, y, z, c, t) = (Image::value_type)(sum / window_size);                             //assign the current pixel value as the mean of the window pixels  
                            }
			            }
		            }
	            }
